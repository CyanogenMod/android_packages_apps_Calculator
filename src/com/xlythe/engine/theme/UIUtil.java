...
public class UIUtil {
	...
	public static Drawable flipDrawable(Context context, Drawable original) {
		...
		// sprite
		Bitmap fSprite = Bitmap.createBitmap(orig, 0, 0, orig.getWidth(), orig.getHeight(),
				mirrorMatrix, false);
	...
	public static void measureView(View v) {
		if(v.getHeight() == 0) {
			v.measure(MeasureSpec.makeMeasureSpec(getWindowWidth(v.getContext()),
					MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(Integer.MAX_VALUE,
					MeasureSpec.AT_MOST));
	...
	private static void layoutAllViews(View v, int l, int t, int r, int b) {
		...
		if(ViewGroup.class.isAssignableFrom(v.getClass())) {
			...
			for(int i = 0; i < fl.getChildCount(); i++) {
				...
				layoutAllViews(child, child.getLeft(), child.getTop(), child.getRight(),
						child.getBottom());
			}
		}
	}
