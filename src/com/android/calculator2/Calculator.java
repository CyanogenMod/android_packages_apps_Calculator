...
public class Calculator extends Activity implements Logic.Listener, OnClickListener,
	...
	public void onCreate(Bundle state) {
		...
		if(sliderBackground == null) sliderBackground = Theme.getDrawable(getContext(),
		...
		if(mPager != null) {
			...
			CalculatorPageAdapter largeAdapter = new LargePageAdapter(getContext(), mListener,
					mGraph, mLogic);
	...
	public void showFirstRunSimpleCling(boolean animate) {
		...
		if(isClingsEnabled()
				&& !CalculatorSettings.isDismissed(getContext(), Cling.SIMPLE_CLING_DISMISSED_KEY)) {
	...
	public void showFirstRunMatrixCling(boolean animate, View matrixPage) {
		// Enable the clings only if they have not been dismissed before
		if(isClingsEnabled()
				&& !CalculatorSettings.isDismissed(getContext(), Cling.MATRIX_CLING_DISMISSED_KEY)) {
	...
	private void updateDetails() {
		if(mDetails != null && CalculatorSettings.showDetails(getContext())) {
			String text = "";
			String units = CalculatorSettings.useRadians(getContext()) ? getString(R.string.radians)
