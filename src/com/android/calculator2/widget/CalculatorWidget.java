...
public class CalculatorWidget extends AppWidgetProvider {
    ...
    private static String addOperator(String equation, char op) {
        ...
        if (equation.length() == 0) {
            ...
            if ((lastChar == getDecimal()) ||
                    (lastChar == Constants.MINUS && lastlastChar == Constants.MINUS) ||
                    (lastChar == Constants.MINUS && op == Constants.MINUS && Solver.isOperator(lastlastChar))) {
                //Nothing
            }  else if(Solver.isOperator(lastlastChar) && Solver.isOperator(lastChar)) {
                equation = equation.substring(0, equation.length() - 2);
                equation += op;
            } else if(Solver.isOperator(lastChar) && op != Constants.MINUS) {
