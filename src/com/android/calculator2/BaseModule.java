...
public class BaseModule {
    ...
    BaseModule(Logic logic) {
        ...
        REGEX_NOT_NUMBER = "[^A-F0-9" + Pattern.quote(mLogic.mDecimalPoint)
                + SELECTION_HANDLE + "]";
    ...
    public String groupDigits(String number, Mode mode, int selectionHandle) {
        ...
        switch (mode) {
            case DECIMAL:
                modifiedNumber = group(wholeNumber,
                        mLogic.mDecSeparatorDistance, mLogic.mDecSeparator);
                break;
            case BINARY:
                modifiedNumber = group(wholeNumber,
                        mLogic.mBinSeparatorDistance, mLogic.mBinSeparator);
                break;
            case HEXADECIMAL:
                modifiedNumber = group(wholeNumber,
                        mLogic.mHexSeparatorDistance, mLogic.mHexSeparator);
                break;
        }
