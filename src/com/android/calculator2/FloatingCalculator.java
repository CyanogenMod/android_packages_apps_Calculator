...
public class FloatingCalculator extends Service implements OnTouchListener {
	...
	public static FloatingCalculator ACTIVE_CALCULATOR; // A copy of the service for the floating
														// activity
	...
	private WindowManager.LayoutParams addView(View v, int x, int y, int gravity, int width,
			int height) {
	...
	private boolean isDeleteMode(int x, int y) {
		...
		boolean horz = x + (mDraggableIcon == null ? 0 : mDraggableIcon.getWidth()) > (screenWidth / 2 - boxWidth / 2)
	...
	public void onCreate() {
		...
		// Set up theme engine (the display uses it, but most of it should be turned off. this is
		// just in case)
		...
		mBroadcastReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent myIntent) {
				...
				if(y >= getScreenHeight() - mDraggableIcon.getHeight()) y = getScreenHeight()
						- mDraggableIcon.getHeight() - MARGIN_VERTICAL;
	...
	public void closeCalculator(boolean returnToOrigin) {
		if(mIsCalcOpen) {
			...
			if(FloatingCalculatorActivity.ACTIVE_ACTIVITY != null) FloatingCalculatorActivity.ACTIVE_ACTIVITY
					.finish();
			hideCalculator();
		}
	}
	...
	public void showCalculator() {
		if(mCalcView == null) {
			...
			FloatingCalculatorPageAdapter adapter = new FloatingCalculatorPageAdapter(getContext(),
					mListener, mHistory, mLogic, mDisplay);
	...
	private class AnimationTask {
		...
		private void setup(int x, int y) {
			if(mIsAnimationLocked) throw new RuntimeException(
					"Returning to user's finger. Avoid animations while mIsAnimationLocked flag is set.");
			mDestX = x;
			mDestY = y;
		...
		private int calculateX() {
			float velocityX = calculateVelocityX();
			int screenWidth = getScreenWidth();
			int destX = (mParams.x + mDraggableIcon.getWidth() / 2 > screenWidth / 2) ? screenWidth
					- mDraggableIcon.getWidth() - MARGIN_HORIZONTAL : 0 + MARGIN_HORIZONTAL;
		...
		private int calculateY() {
			...
			if(destY >= screenHeight - mDraggableIcon.getHeight()) destY = screenHeight
					- mDraggableIcon.getHeight() - MARGIN_VERTICAL;
			return destY;
		}
